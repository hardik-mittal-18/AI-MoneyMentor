// @ts-nocheck
// Canonical implementation lives in BrokerDemoPanel.jsx; this file forwards to it.
export { BrokerDemoPanel } from "./BrokerDemoPanel.jsx";
...
const LOCAL_DEMO_PORTFOLIO: BrokerPortfolioResponse = {
  user: "Demo User",
  portfolio: [
...
function MetricCard({ label, value, hint, valueClassName }: { label: string; value: string; hint?: string; valueClassName?: string }) {
  return (
...
function seedHistory(base: number, points = 12): PricePoint[] {
  const now = Date.now();
  let p = Number.isFinite(base) && base > 0 ? base : 100;
  const out: PricePoint[] = [];
  for (let i = points - 1; i >= 0; i--) {
    const t = new Date(now - i * 2000).toLocaleTimeString("en-IN", {
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit",
    });
    p = p * (1 + (Math.random() - 0.5) * 0.01);
    out.push({ t, price: Math.round(clamp(p, 1, 1_000_000) * 100) / 100 });
  }
  return out;
}

export function BrokerDemoPanel({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  const [analysis, setAnalysis] = useState<AnalyzePortfolioResponse | null>(null);
  ...
  const availableStocks = useMemo(
    () => ["RELIANCE", "TCS", "INFY", "HDFCBANK", "ICICIBANK", "SBIN", "ITC", "HINDUNILVR"] as const,
    [],
  );
  const [selectedStock, setSelectedStock] = useState<(typeof availableStocks)[number]>("RELIANCE");
  ...
  const [pricesByStock, setPricesByStock] = useState<Record<string, number>>({});
  ...
  async function refreshBrokerBalance() {
    try {
      ...
      // Ignore balance refresh failures in demo mode.
    }
  }
  ...
  useEffect(() => {
    // reset chart series when stock changes
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      const nextMs = 2000 + Math.floor(Math.random() * 3000); // 2-5 sec
      timerRef.current = window.setTimeout(() => {
        ...
        setPricesByStock((prev) => {
          ...
          for (const stock of availableStocks) {
            ...
            next[stock] = Math.round(clamp(updated, 1, 1_000_000) * 100) / 100;
          }
        ...
        setHistory((prev) => {
          ...
          const point: PricePoint = {
            ...
            price: Math.round(clamp(safeNext, 1, 1_000_000) * 100) / 100,
          };
  ...
  function applyPortfolioAndStartLive(data: BrokerPortfolioResponse, modeNotice?: string) {
    setPortfolio(data);
    setStatus("connected");
    ...
    setPricesByStock(() => {
      const next: Record<string, number> = {};
      for (const stock of availableStocks) {
        const found = (data?.portfolio as any[])?.find((x) => String(x.stock).toUpperCase() === stock);
    ...
    const base = (data?.portfolio as any[])?.find((x) => String(x.stock).toUpperCase() === selectedStock)?.price ?? basePrices[selectedStock];
    setHistory(seedHistory(Number(base), 12));
  }

  async function connectBrokerDemo(options?: { isAuto?: boolean }) {
    ...
    try {
      ...
      const parsed = data as DashboardResponse;
      const brokerResponse: BrokerPortfolioResponse = {
        user: (user?.email ?? "Demo User") as any,
      ...
      if (/Failed to fetch|NetworkError/i.test(rawMessage)) {
        // Fallback: keep the dashboard usable even without backend.
        const local = {
  ...
  async function runPortfolioAnalysis() {
    ...
    try {
      ...
      setAnalysis(data as AnalyzePortfolioResponse);
      ...
      const message = /Failed to fetch|NetworkError/i.test(rawMessage)
        ? `API not reachable. Start the backend on ${apiBaseUrl}.`
        : rawMessage;
  ...
  useEffect(() => {
    // Auto-load dashboard data so the page never feels empty.
  ...
  return (
    <Card className="rounded-2xl">
      ...
      <CardContent className="space-y-6">
        ...
        {loading && !portfolio ? (
          <Alert className="animate-in fade-in-0 duration-200">
                ...
                <CardHeader>
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="w-full sm:w-56">
                      <Select value={selectedStock} onValueChange={(v) => setSelectedStock(v as any)}>
                        <SelectTrigger>
              ...
              <div className="grid gap-4 lg:grid-cols-12 animate-in fade-in-0 duration-300">
                ...
                <Card className="rounded-2xl lg:col-span-4">
                  ...
                  <CardContent className="space-y-2">
                    {(analysis.risks.length ? analysis.risks : ["No major risks detected (demo)."])
                      .slice(0, 6)
                      .map((x, idx) => (
                        <div key={`${idx}-${x}`} className="text-sm text-red-600" style={{ fontWeight: 800 }}>
                          • {x}
                        </div>
                      ))}
                  </CardContent>
                ...
                <Card className="rounded-2xl lg:col-span-4">
                  ...
                  <CardContent className="space-y-2">
                    {analysis.suggestions.slice(0, 6).map((x, idx) => (
                      <div key={`${idx}-${x}`} className="text-sm text-blue-600" style={{ fontWeight: 800 }}>
                        • {x}
                      </div>
                    ))}
                  </CardContent>
                ...
                <Card className="rounded-2xl lg:col-span-12">
                  ...
                  <CardContent className="grid gap-2 sm:grid-cols-2">
                    {analysis.opportunities.slice(0, 6).map((x, idx) => (
                      <div key={`${idx}-${x}`} className="text-sm" style={{ fontWeight: 800 }}>
                        • {x}
                      </div>
                    ))}
                  </CardContent>
